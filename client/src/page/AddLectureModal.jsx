...
function AddLectureModal() {
  ...
  useEffect(()=>{
    ...
    console.log("Subsection Data on Edit From Add Lecture Modal=>",sectionData)
  },[clickedSubSectionID,sectionData])
  ...
  const {
    ...
    {
      defaultValues:subSectionData && editOrNextStatusOfVideo===0?{
  ...
  const createSubSectionFunction=(data)=>{
    ...
    try {
      ...
      formData.append('video',data.thumbnail)
  
      console.log("Lecture Data:", formData);

      if(editOrNextStatusOfVideo===0){  //editMode
        console.log("editing subsection",editOrNextStatusOfVideo)
        console.log("FormData=> ",data)
        formData.append('subSectionID',clickedSubSectionID)
